...
export default function LettreOrientationModal({ isOpen, onClose, patient }) {
  return (
    <Dialog open={isOpen} onClose={onClose} className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <Dialog.Panel className="bg-white p-6 rounded-xl shadow-xl w-full max-w-lg">
        <h2 className="text-xl font-bold mb-4">Lettre d'orientation</h2>
        <div id="print-area" className="space-y-4">
          <p>Nom: {patient?.nom} {patient?.prenom}</p>
          <input type="text" placeholder="Confrère" className="w-full border p-2" />
